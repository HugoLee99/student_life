...
def federated_learning(
    ...
    for round in range(num_rounds):
        ...
        for k in clients:
            ...
            if k < len(train_data):  # train_data 是一个包含所有客户端训练数据的列表
                train_graphs, unlabeled_graphs, labels = train_data[k]
            ...
            with torch.no_grad():
                for client in clients:
                    if client < len(train_data):  # 添加索引检查
                        train_graphs, _, labels = train_data[client]
            ...
            with torch.no_grad():
                for client in clients:
                    if client < len(client_batch_data):  # 添加索引检查
                        train_graphs, _, labels = client_batch_data[client]
...
def fedsem_ft(
    ...
    for round in range(num_rounds):
        ...
        # 将 predictions 和 all_labels 转换为张量
        predictions = torch.cat(predictions, dim=0)  # 合并所有预测
        all_labels =  torch.stack(all_labels)# 确保标签在同一设备上
...
def main():
    ...
    # 准备数据
    expert_batches, apprentice_batches, apprentice_data ,expert_data = prepare_data()
    ...
    # 训练专家模型（前40个通信轮次）
    num_rounds_expert = 40
    num_clients = 3  # 修改为3个客户端
    clients = list(range(num_clients))  # 确保clients的索引在有效范围内
